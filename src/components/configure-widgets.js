...
import { WidgetsContext, WidgetsProvider } from './widgets-context';

import Layout from "./layout";
import SEO from "./seo";

export default function Category() {
    const [recentPostsTitle, setRecentPostsTitle] = useState('');

    // const [widgets, setWidgets] = useContext(WidgetsContext);

    let recentPostsTitleHandler = e => {
        const { value } = e.target;
        setRecentPostsTitle(value);
        // setWidgets(prevWidgets => [...prevWidgets, { recentPostsTitle }]);
    }
    ...
    return (
        <Layout>
            ...
            <p>
                ...
                <input className="widefat" type="text" value={recentPostsTitle} onChange={recentPostsTitleHandler}/>
            </p>
        </Layout>
    )
}
